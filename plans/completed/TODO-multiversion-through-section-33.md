# Multi-Version Support — the full TODO as it stood through §33

**Archived 2026-08-25, verbatim and complete.** This is `TODO.md` exactly as it stood at commit
`d5fb36dbf`, immediately before the §34 cleanup trimmed it from 3542 lines back to working size.
Nothing below the rule was edited, reordered or summarised — it is a byte-for-byte copy, so that the
completed sections (§8.3 and §22 – §33), every withdrawn ruling (R-v), and every closed debt row keep
their full reasoning.

**What this archive is for.** The live `TODO.md` now carries the `26.2` port's *state* and its open
work; the sections below carry *how each number was arrived at* — the 2,639 → 0 compile ladder, the
54 → 0 injector re-derivation, the 186 → 1 suite triage, and, more valuable than any of them, the
**five blind spots** those passes found. Read this file before re-deriving a measurement or
re-litigating a call; the summary in `TODO.md` deliberately does not repeat the evidence.

🔑 **The blind spots are the part worth keeping.** Each one is a class of defect that every gate in
this repo reports as green: a mixin that *applies* but binds the wrong live method (§32); a seam that
became a pass-through, where `allow` is only an upper bound (§32); `@Shadow`/`@Accessor` members,
invisible to javac *and* to the allow-audit (§32.6); a narrow overload deleted while a wider one
survives, which javac must accept by the language rules (§33.4); and a Minecraft method name
surviving as a **string literal**, which no renamer parses (§33.5).

Companion archives, covering the earlier work: `TODO-multiversion-through-phase-21.md` and
`TODO-multiversion-phases-0-7.md`.
