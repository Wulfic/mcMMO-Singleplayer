...
| **Lead Lungs** | Swimming | 300 | Restores up to **0.75 air ticks per tick** while submerged. Vanilla drains 1/tick, so breath gets very long but never infinite — the cap is deliberately below 1.0. |
| **Lake Raider** | Swimming | 1000 | Up to a **15 % chance** for an underwater block break to turn up treasure. |
| **Glide** | Flying | 300 | Descend up to **50 % more slowly** while gliding. Capped well below 1.0 so you can still land. |
| **Solar Wings** | Flying | 1000 | A worn elytra repairs **1 durability per 100 ticks** in daylight, **×2 while grounded**. Deliberately a trickle — make it generous and elytra durability stops being a resource. |
| **Fleet Footed** | **each of the three** | **1** | Move faster through that medium. Scales with the skill that owns it. |
| **Second Wind** | **each of the three** | **50** | The super ability — see below. |

Unlock levels are RetroMode, which is the shipped default. In Standard mode divide by ten (so Fleet Footed is still 1, and Second Wind is 5).

> **Second Wind moved from 250 to 50 in GitHub #17.4.** Every other super ability in the mod unlocks at Standard 5 — Super Breaker, Tree Feller, Giga Drill Breaker, Serrated Strikes, Skull Splitter and Green Terra all do — and the movement super was five times later than any of them. The three mediums still share one value, which is the property that matters.
...
Now a pure flier gets air Fleet Footed at Flying **1** and the air Second Wind at Flying **50**, and a pure swimmer the same in water. Being an all-rounder still earns you *more* — all six copies instead of two — but it is no longer a **gate** on any single one of them.
...
Each body unlocks at **50** in its own skill, independently of the other two — reach Swimming 50 and you have the water body whether or not you have ever flown.
