...
| | |
|---|---|
| **Minecraft** | 1.21.11 and 26.1 – 26.3 under current development; 1.21 – 1.21.10 on the **archived** v1.4.0 jars — see [Supported versions](#supported-versions) |
...
These six bands are **no longer updated**, and their **jars are still published and still work**. v1.4.0 is their final release: download it from the [Releases page](https://github.com/Wulfic/mcMMO-Singleplayer/releases) exactly as before, and install it exactly as described below. What stopped is new development — later fixes and features land on the bands in the table above.
...
**Minecraft 1.21.10 and older are not supported** by current development — `1.21` through `1.21.10` are served by the **archived** bands above, whose final release is v1.4.0 and whose jars stay downloadable. Below `1.21` there has never been a build. The whole `26.x` line **is** current: three bands in the first table above cover `26.1`, `26.1.1`, `26.1.2`, `26.2` and `26.3`.
