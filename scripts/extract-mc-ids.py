#!/usr/bin/env python3
"""Generate `scripts/mc-ids.txt` -- every vanilla item, block and entity-type registry id, per
Minecraft version.
...
# The registry each kind is dumped under. NOT derivable from the kind name: the entity registry is
# `minecraft:entity_type`, not `minecraft:entity`. This map exists so the kind stays the short token
...
HEADER = """\
# mc-ids.txt -- vanilla item, block and entity-type registry ids, per Minecraft version.
#
...
#
# ⚠️ The 'entity' kind is the entity_type registry (§52). Unlike blocks and items it has NO jar-asset
# counterpart, so it is generated but never cross-validated -- see cross_validate().
"""
...
def default_versions(all_cached: bool) -> list[str]:
    """Which versions a plain run regenerates.

    🔴 THE DEFAULT IS THE MANIFEST'S OWN VERSION LIST, NOT EVERY CACHED VERSION, and §52 is why.
    A plain run on this machine wanted to add NINE versions the manifest deliberately excludes --
    the entire `1.20` line, which ruling R-x withdrew from scope, plus `26.1`/`26.1.1`. Nothing
    was wrong with the dump; the default simply took `cached_versions()`, which is an accident of
    what Loom happened to download rather than a statement about what this project supports.

    Two things that made it dangerous rather than merely noisy:

    * **`--write` would have silently widened the declared scope**, and the diff that discloses it
      is 9 lines in a 23-line report that otherwise looks like the routine `entity +150` rows.
    * **`--check` could never pass** on a developer machine holding one extra cached version. A gate
      that is red for a reason unrelated to the thing it grades gets ignored, then removed.

    Widening scope is now the OPT-IN (`--all-cached`), which is the right shape: adding a band is a
    deliberate act with a ruling behind it, and it should have to say so on the command line.
    """
    manifest = load_manifest()
    cached = cached_versions()
    if all_cached:
        print(f"--all-cached: every cached version ({len(cached)}), including any the manifest does "
              f"not carry yet.\n")
        return cached
    if not manifest:
        print(f"No manifest yet -- generating for every cached version ({len(cached)}).\n")
        return cached

    known = [v for v in cached if v in manifest]
    absent = sorted(set(manifest) - set(cached), key=version_key)
    extra = sorted(set(cached) - set(manifest), key=version_key)
    print(f"Regenerating the {len(known)} version(s) the manifest already carries. "
          f"Use --all-cached to widen scope.")
    if absent:
        # Loud, because it means the run cannot re-verify part of the manifest and a later --check
        # would grade those versions against nothing.
        print(f"  WARNING: {len(absent)} manifest version(s) have NO cached server jar and are "
              f"being left untouched: {', '.join(absent)}")
    if extra:
        print(f"  note: {len(extra)} cached version(s) are NOT in the manifest and are being "
              f"skipped: {', '.join(extra)}")
    print()
    return known
...
def cross_validate(version: str, registry: dict[str, set[str]],
    ...
    ⚠️ ENTITY IS DELIBERATELY NOT CROSS-CHECKED, and that is stated here rather than left to the
    reader of the loop below. There is no asset counterpart for the entity registry -- entities have
    no `assets/minecraft/items/` or `blockstates/` tree -- so there is nothing to compare against.
    The dangerous shape would be folding ENTITY into a generic loop over KINDS: `assets[ENTITY]`
    would be an empty set, every id would read as "in the registry but not in assets", and the
    ...
    for v in versions:
        ...
        if assets is None:
            ...
            print(f"      {len(registry[BLOCK]):5} blocks {len(registry[ITEM]):5} items "
                  f"{len(registry[ENTITY]):4} entities "
            ...
            print(f"      {len(registry[BLOCK]):5} blocks {len(registry[ITEM]):5} items "
                  f"{len(registry[ENTITY]):4} entities "
                  f"[blocks+items cross-checked against the jar assets: exact; "
                  f"entities not cross-checked -- no asset source]")
        data[v] = registry
    return data
...
def self_test() -> int:
    ...
    sample = {
        "1.21": {BLOCK: {"stone", "dirt"}, ENTITY: {"zombie", "cow"},
                 ITEM: {"stick", "stone"}},
        "1.21.11": {BLOCK: {"stone", "dirt", "iron_chain"}, ENTITY: {"zombie", "cow", "happy_ghast"},
                    ITEM: {"stick", "stone", "iron_chain"}},
    }
    ...
    reg = {ITEM: {"stick", "stone"}, BLOCK: {"stone", "dirt"}, ENTITY: {"zombie"}}
    ...
    # --- §52's entity kind. Three separate claims, because they fail in different directions.
    #
    # (1) The registry id is NOT derivable from the kind token. `minecraft:entity` does not exist;
    #     getting this wrong makes registry_ids() raise on every version, which is at least loud.
    check("entity registry id", REGISTRY_ID[ENTITY] == "minecraft:entity_type",
    ...
    check("entities do not break the cross-check", cross_validate("t", reg, assets_ok) == [],
          "a registry carrying entities was reported as disagreeing with the jar assets")

    # (3) ...but an assets dict that somehow carries ENTITY is a BUG, not new coverage, and must
    #     refuse rather than silently compare against a set that has no source.
    try:
        cross_validate("t", reg, dict(assets_ok, **{ENTITY: {"zombie"}}))
        failures.append("entity assets: cross_validate accepted an ENTITY asset set")
...
def main() -> int:
    ...
    ap.add_argument("--mc", action="append", metavar="VERSION",
                    help="generate only this version (repeatable); default is every version the "
                         "manifest already carries")
    ap.add_argument("--all-cached", action="store_true",
                    help="generate for every Loom-cached version, ADDING any the manifest does not "
                         "carry yet. This widens the declared scope -- the opt-in for a new band")
    ...
    versions = args.mc or default_versions(args.all_cached)
    if not versions:
        raise SystemExit(
