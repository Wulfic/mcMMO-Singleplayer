...
class MixinApplicationTest {
    ...
    void mobOriginMixinsApply() {
        ...
        // ⚠️ The band's second half of the same gate, and it exists only on mc/1.21.4. This band's
        // newest available fabric-api ships data-attachment-api 1.6.2, whose
        ...
        assertTrue(Arrays.stream(EntityType.class.getDeclaredMethods())
                ...
                "EntityTypeSpawnOriginMixin's getEntityFromNbt re-stamp did not apply to EntityType "
                        ...
                        + "destroyed by fabric-api 1.6.2's attachment read, so /summon, mob "
                        + "spawners and chunk load would all produce mobs that read as NATURAL and "
                        + "the egg-farm exploit gate would be open");
