...
/**
 * Stores hash tables for item and block names, keyed by their vanilla registry-id path (e.g.
 * {@code diamond_axe}). Pure {@link HashSet}/{@link HashMap} membership — no Minecraft types — so
 * it is the MC-free classification core that {@code ItemUtils}/{@code BlockUtils} delegate to once
 * they extract the id path from a live item/block. Singleplayer port: the legacy Bukkit
 * {@code Material} convenience overloads are dropped (callers always hold the registry-path
 * String); every classifier is the String form.
 */
public class MaterialMapStore {
    ...
    public boolean isMultiBlockPlant(@NotNull String id) {
        return multiBlockPlant.contains(id);
    }

    public boolean isMultiBlockHangingPlant(@NotNull String id) {
        return multiBlockHangingPlant.contains(id);
    }

    public boolean isAbilityActivationBlackListed(@NotNull String id) {
        return abilityBlackList.contains(id);
    }

    public boolean isToolActivationBlackListed(@NotNull String id) {
        return toolBlackList.contains(id);
    }

    public boolean isMossyWhiteListed(@NotNull String id) {
        return mossyWhiteList.contains(id);
    }
    ...
    public boolean isHerbalismAbilityWhiteListed(@NotNull String id) {
        return herbalismAbilityBlackList.contains(id);
    }

    public boolean isBlockCrackerWhiteListed(@NotNull String id) {
        return blockCrackerWhiteList.contains(id);
    }
...
//        intendedToolPickAxe.add("stairs");
    ...
    public boolean isGlass(@NotNull String id) {
        return glassBlocks.contains(id);
    }

    public boolean isFood(@NotNull String id) {
        return foodItemWhiteList.contains(id);
    }
