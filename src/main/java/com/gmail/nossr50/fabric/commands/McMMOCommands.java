...
public final class McMMOCommands {
     ...
     * has reworked the command permission API more than once, and both the helper's <i>return
     * type</i> and its <i>existence</i> vary by version:
     *
     * <ul>
     ...
     * <code>≤1.21.10</code>. <b>That was false on this band</b> — 1.21.5 has neither overload.
     * Verified with {@code javap} against the band's own merged jar, not recalled. The tiers above
     * are deliberately named by <em>relation</em> rather than pinned to version numbers: a comment
     * asserting what a version has is read by no compiler and rots in silence.
     */
