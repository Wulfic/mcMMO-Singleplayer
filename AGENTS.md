...
| `scripts/probe-bands.py` | which MC symbols differ on a version (`--control` guards it). ⚠️ **The record count is a PER-BAND fact** — do not quote one here. ✅ **`--check` is ship gate 12** (§56.4): read-only, no `--out` write, and it validates the manifest against **every version in `supported_minecraft_versions`**, not just `minecraft_version`. 🔑 A band ships a RANGE and the control used to be ONE version, so `mc/26.1.2` validated `26.1.2` and shipped to `26.1` unchecked. ⚠️ An ABSENT on the primary is a bug in the PROBE; on any other declared version it is a real SHIPPING defect — the script names which. ⚠️ It refuses `--allow-control-failures`, and an uncached declared version is refused, never skipped |
